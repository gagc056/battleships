...
const strokeSelection = (
  ...
  beyondRow, beyondColumn
  ) => {

  const x1 = originX + (squareLength * startColumn);
  const x2 = originX + (squareLength * beyondColumn);
  const y1 = originY + (squareLength * startRow);
  const y2 = originY + (squareLength * beyondRow);
...
const NOTHING_ICON = 0;
const TARGET_SCOPE_ICON = 1;
const WATER_DROPLET_ICON = 2;
const EXPLOSION_ICON = 3;

const mouseDownChangeScopeFactory = (canvas, battleCanvas) => {
  return ((event) => {
...
const battleCanvasFactory = (canvas, gameboard) => {
  return new BattleCanvas(canvas, gameboard);
};
