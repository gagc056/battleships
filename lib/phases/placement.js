...
const placementStartFactory = (buttons, bars, form, canvas, icons, nextPhase) => {
  ...
  return((gameLoop) => {
    ...
    form.shipRow.onchange = () => {
      state.row = Number.parseInt(form.shipRow.value);
      update();
    };

    form.shipColumn.onchange = () => {
      state.column = Number.parseInt(form.shipColumn.value);
      update();
    };
    ...
    placeShip.onclick = () => {
      if (state.shipsLeft > 0) {
        ...
        const row = Number.parseInt(form.shipRow.value);
        const column = Number.parseInt(form.shipColumn.value);
